        ...
        <div class="card seeker-admin-card">
            <div class="table-responsive">
                <table class="table table-hover align-middle seeker-admin-table seeker-admin-table--actions mb-0">
                    <thead><tr><th>@lang('seeker::admin.publication')</th><th>@lang('seeker::admin.author')</th><th>@lang('seeker::admin.status')</th><th>@lang('seeker::admin.publications.dates')</th><th class="text-center">@lang('seeker::admin.conversation_count')</th><th class="text-end">@lang('seeker::admin.actions')</th></tr></thead>
                    <tbody>
                                ...
                                <td class="text-end text-nowrap">
                                    <div class="d-flex justify-content-end align-items-center gap-2">
                                        @unless($publication->trashed())
                                            <div class="seeker-admin-status-actions" role="group" aria-label="@lang('seeker::admin.set_status')">
                                                @foreach([
                                                    \Azuriom\Plugin\Seeker\Models\Publication::STATUS_ACTIVE => ['check-lg', 'success'],
                                                    \Azuriom\Plugin\Seeker\Models\Publication::STATUS_CLOSED => ['x-lg', 'secondary'],
                                                    \Azuriom\Plugin\Seeker\Models\Publication::STATUS_HIDDEN => ['eye-slash', 'danger'],
                                                    ...
                                                    <form method="POST" action="{{ route('seeker.admin.publications.status', $publication) }}">
                                                        ...
                                                        <button class="btn btn-sm {{ $publication->status === $publicationStatus ? 'btn-'.$statusColor : 'btn-outline-'.$statusColor }}"
                                                                title="@lang('seeker::admin.publications.change_status', ['status' => trans('seeker::messages.statuses.'.$publicationStatus)])"
                                                                aria-label="@lang('seeker::admin.publications.change_status', ['status' => trans('seeker::messages.statuses.'.$publicationStatus)])"
